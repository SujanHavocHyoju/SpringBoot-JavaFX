...
public class ImageUtils {
    ...
    public static File toPdf(String pdfFolder, String prefixProperty, String rocketIdFromWeb, String rocketId, List<File> imageFilesToMerge) throws IOException, DocumentException {
        ...
        int compressionLevel = Integer.MAX_VALUE;
...
//                }
    ...
    public static File toPlainPdf(String pdfFolder, String prefixProperty, String rocketId, List<File> imageFilesToMerge) throws IOException, DocumentException {
        ...
        for (File file : imageFilesToMerge) {
            size += com.solar.utils.FileUtils.calculateDirectoryInMegaBytes(file);
        }
        File pdfFile = new File(dir, fileName);
...
//            }
    ...
    public static File toSingleImagePdf(String pdfFolder, String prefixProperty, String solarId, File file) throws IOException, DocumentException {
        String extension = ".pdf";
        String fileName = prefixProperty + solarId + extension;
        File dir = new File(pdfFolder + File.separator + solarId);
        if (!dir.exists()) {
            dir.mkdir();
        }
        File pdfFile = new File(dir, fileName);
    ...
    public static String spaceBetweenEachLetter(String rocketIdFromWeb) {
        //String lastFourDIgit = rocketIdFromWeb.substring(rocketIdFromWeb.length() - 4);
        StringBuilder str = new StringBuilder("\n\n\n\n");
        for (char c : rocketIdFromWeb.trim().toCharArray()) {
            str.append(c).append(" ");
        }
        return str.append(" ")
                .append(" ")
    ...
    private static byte[] loadFile(File file) throws IOException {
        ...
        if (length > Integer.MAX_VALUE) {
            throw new SolarFileNotFoundException("Big file");
        }
        ...
        if (offset < bytes.length) {
            throw new SolarFileNotFoundException("Could not completely read file " + file.getName());
        }
